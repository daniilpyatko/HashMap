...
private:
    ...
    vector < vector < pair < const KeyType, ValueType > > > all;
    Hash hasher;
...
public:
    ...
    HashMap(std::initializer_list<pair<const KeyType, ValueType>> init, Hash nhasher = Hash()): hasher(nhasher){
        all.resize(capacity);
        auto b = init.begin();
        auto s = init.end();
        while(b != s){
    ...
    public:
        ...
        bool operator!= (const iterator &other){return !(pos == other.pos);}
        iterator operator++ (int asd){iterator tmp = iterator(pos, h); pos = h->findnext(pos.first, pos.second);return tmp;}
        iterator operator++ (){pos = h->findnext(pos.first, pos.second);return *this;}
    ...
    iterator begin() {return iterator(findnext(0, -1), this);}
    iterator end() {return iterator({capacity, -1}, this);}
    ...
    public:
        const_iterator(){}
        const_iterator(pair < int, int > _pos, const HashMap* _h): pos(_pos), h(_h){}
        bool operator== (const const_iterator &other){return pos == other.pos;}
        bool operator!= (const const_iterator &other){return !(pos == other.pos);}
        const_iterator operator++ (int asd){const_iterator tmp = const_iterator(pos, h); pos = h->findnext(pos.first, pos.second);return tmp;}
        const_iterator operator++ (){pos = h->findnext(pos.first, pos.second);return *this;}
    ...
    const_iterator begin() const {return const_iterator(findnext(0, -1), this);}
    const_iterator end() const {return const_iterator({capacity, -1}, this);}
    ...
    void rebuild(){
        ...
        vector < vector < pair < const KeyType, ValueType > > > nall(capacity);
        for(size_t i = 0; i < all.size(); ++i){
    ...
    iterator find(KeyType x){
        ...
        for(size_t i = 0; i < all[ind].size(); ++i){
            if(x == all[ind][i].first){
                return iterator({(int)ind, i}, this);
            }
        }
        return end();
    }
    ...
    const_iterator find(KeyType x) const {
        ...
        for(size_t i = 0; i < all[ind].size(); ++i){
            if(x == all[ind][i].first){
                return const_iterator({(int)ind, i}, this);
            }
        }
        return end();
    }

...
class HashMap {
 ...
 public:
  HashMap(Hash nhasher = Hash()):hasher(nhasher) { all.resize(capacity); }
  ...
  HashMap(std::initializer_list<pair<const KeyType, ValueType> > init,
          Hash nhasher = Hash()):hasher(nhasher) {
    all.resize(capacity);
    auto b = init.begin();
    auto s = init.end();
    while (b != s) {
  ...
  /*
    Expand condition: cnt_elements > capacity * 2
    Shrink condition: cnt_elements < capacity / 4
    New capacity is the smallest number such that it is at least min_capacity and at least cnt_elements * 2
  */
  void rebuild() {
    int newcapacity = max(static_cast<size_t>(4), cnt_elements * 2);
  ...
  int size() const { return cnt_elements; }

  bool empty() const { return cnt_elements == 0; }
};
